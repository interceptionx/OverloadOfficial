...
public class Party {
    ...
    public static final List<Item> HP_REWARDS = Arrays.asList();
    ...
    public void addPersonalPoints(Player player, int points) {
        boolean darkMagicianPet = player.hasPetOut("Dark magician");
        ...
        if(centaurPet || olmletPet || darkMagicianPet) {
            percentageBoost += 10;
        }
    ...
    public static void startRaid(Player p) {
        ...
        if(party.getRaidsSelected() == RaidsType.CHAMBER_OF_SECRETS) {
            p.message("The custom raid is under construction. Please choose another.");
            return;
        }
