...
public class UnlockEventRewards {
    ...
    public void reset() {
        ...
        if (unlockedAllRewards) {
            player.optionsTitled("Would you like to reset all the rewards?", "Yes", "No", () -> {
                ...
                World.getWorld().sendWorldMessage( Color.MEDRED.wrap("[Winter event]:") + "</shad> " + Color.PURPLE.wrap(player.getUsername()) + " has just reset the " + Color.PURPLE.wrap("Winter event") + " rewards!");
    ...
    private final List<Integer> items_to_shout = Arrays.asList(
        DONATOR_MYSTERY_BOX, ARMADYL_GODSWORD, LEGENDARY_MYSTERY_BOX, ZENYTE_MYSTERY_BOX, RAIDS_MYSTERY_BOX, MYSTERY_TICKET, KEY_OF_DROPS, REVENANT_MYSTER_BOX, CHRISTMAS_CRACKER, WINTER_ITEM_CASKET);
    ...
    public void rollForReward(int tokens, int amount, Item reward) {
        ...
        if (shout) {
            ...
            World.getWorld().sendWorldMessage(Color.MEDRED.wrap("[News]:") + "</shad> " + Color.PURPLE.wrap(player.getUsername()) + " received " + Color.HOTPINK.wrap("x" + reward.getAmount()) + " " + Color.HOTPINK.wrap(reward.unnote().name()) + " from the Event!");
        } else {
