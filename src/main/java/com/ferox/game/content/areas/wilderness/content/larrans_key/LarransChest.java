    ...
    public boolean handleObjectInteraction(Player player, GameObject obj, int option) {
        if (obj.getId() == 34832) {
            ...
            if (player.inventory().contains(LARRANS_KEY_TIER_I)) {
                ...
                player.message("This molten chest wont budge, I think I need to find a key that fits.");
            }
            return true;
        }
        return false;
    ...
    private static void open(Player player, int key) {
        ...
        Chain.bound(player).runFn(1, () -> {
            ...
            for (int i = 0; i < roll; i++) {
                ...
                World.getWorld().sendWorldMessage("[<shad><col=" + Color.RED.getColorValue() + ">Larran's chest</shad></col>]: " + "<col=000000>" + player.getUsername() + " has just looted the Larran's chest with a Larran's key!");
                ...
                if (reward.getValue() >= 30_000) {
                    ...
                    String msg = "[<shad><col=" + Color.RED.getColorValue() + ">Larran's chest</shad></col>]: " + "<col=000000>" + player.getUsername() + " has received " + amtString + " " + reward.unnote().name() + "!";
