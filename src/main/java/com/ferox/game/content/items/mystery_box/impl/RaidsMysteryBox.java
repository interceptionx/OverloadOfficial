    ...
    private void open(Player player) {
        if (player.inventory().contains(RAIDS_MYSTERY_BOX)) {
            ...
            if(broadcast && !player.getUsername().equalsIgnoreCase("Box test")) {
                String worldMessage = "<img=1863><shad=0>[<col=" + Color.RED.getColorValue() + ">Raids mystery box</col>]</shad=0>:<col=FF0000> " + player.getUsername() + " received a <shad=0>" + reward.name() + "</shad>!";
