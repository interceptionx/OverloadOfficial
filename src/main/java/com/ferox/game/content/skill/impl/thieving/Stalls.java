...
public class Stalls extends PacketInteraction {
    ...
    public enum Stall {
        ...
        MONKEY_GENERAL_STALL(25, 2, 36.0, 49000, "general stall", new int[][]{{4876, 4797},}),
        MAGIC_STALL(50, 2, 60, 12000, "magic stall", new int[][]{{4877, 4797},}),
        SCIMITAR_STALL(75, 2, 100.0, 1000, "scimitar stall", new int[][]{{4878, 4797},});
    ...
    private void attempt(Player player, Stall stall, GameObject object, int replacementID) {
        ...
        Chain.bound(player).runFn(1, () -> {
            ...
            if (Utils.percentageChance(5)) {
                TaskManager.submit(new ForceMovementTask(player, 3, new ForceMovement(player.tile().clone(), new Tile(3, 0), 0, 70, 2)));
                player.animate(3130);
