...
public class EarningPotential {
    ...
    /**
     * Hotspots are: Edgeville, Mage bank and Cave of death
     *
    ...
    public static void randomPotentialDrop(Player player) {
        ...
        if (World.getWorld().random(1000) <= (1 + totalPotential / 100)) {
            ...
            World.getWorld().sendWorldMessage("<img=1838>[<col=" + Color.MEDRED.getColorValue() + ">Earning Potential</col>]: <col=800000>" + player.getUsername() + "<col=ff0000> has received an <col=800000>extremely rare<col=ff0000> bonus drop of: <col=ff00ff>" + myDrop.name() + "<col=ff0000>!");
        ...
        if (World.getWorld().random(550) <= (1 + totalPotential / 100)) {
            ...
            player.inventory().addOrBank(myDrop);
            World.getWorld().sendWorldMessage("<img=1838>[<col=" + Color.MEDRED.getColorValue() + ">Earning Potential</col>]: <col=800000>" + player.getUsername() + "<col=ff0000> has received a <col=800000>very rare<col=ff0000> bonus drop of: <col=ff00ff>" + myDrop.name() + "<col=ff0000>!");
            player.putAttrib(AttributeKey.EARNING_POTENTIAL, 0);
            player.getPacketSender().sendString(53722, "Earning Potential: <col=65280>" + player.<Integer>getAttribOr(AttributeKey.EARNING_POTENTIAL, 0) + "%");
        }
